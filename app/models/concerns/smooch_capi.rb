...
module SmoochCapi
  ...
  module ClassMethods
    ...
    def capi_send_message_to_user(uid, text, extra = {}, _force = false, preview_url = true)
      ...
      response = http.request(req)
      Rails.logger.info("[Smooch Bot] [WhatsApp Cloud API] Sending message to #{uid} for number #{self.config['capi_phone_number_id']}. URL: #{uri.to_s} Request: #{payload.to_json}; Response: #{response.body}")
