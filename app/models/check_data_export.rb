class CheckDataExport < ApplicationRecord
  ...
  after_create :request_notification, if: proc { |de| de.status == 'requested' }
  ...
  def request_notification
    options = {
      ...
      requested_on: self.created_at,
    }
    SunsetMailer.delay.request_export_notification(YAML.dump(options))
  end
