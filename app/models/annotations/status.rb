class Status
  ...
  notifies_slack on: :save,
                 ...
                 message: proc { |s| data = s.annotated.data(s.context); "<#{s.origin}/user/#{s.current_user.id}|*#{s.current_user.name}*> changed the verification status on <#{s.origin}/project/#{s.context_id}/media/#{s.annotated_id}|#{data['title']}> from *#{s.id_to_label(s.previous_annotated_status)}* to *#{s.id_to_label(s.status)}*" },
