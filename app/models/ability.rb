class Ability
  ...
  def owner_perms
    ...
    can :destroy, DynamicAnnotation::Field do |obj|
      obj.annotation.get_team.include? @context_team.id && !obj.annotated_is_archived?
    end
  ...
  def journalist_perms
    ...
    can :update, [Media, Link, Claim], { projects: { team: { team_users: { team_id: @context_team.id }}}, archived_was: false }
  ...
  def contributor_perms
    ...
    can :update, [Media, Link, Claim], { user_id: @user.id, archived_was: false }
    can :update, [Media, Link, Claim] do |obj|
      obj.get_team.include? @context_team.id and (obj.user_id == @user.id) and !obj.is_archived?
    end
    can :update, Embed
    ...
    can :create, Flag, ['annotation_type = ?', 'flag'] do |flag|
      flag.get_team.include? @context_team.id and (['Spam', 'Graphic content'].include?flag.flag.to_s) and !obj.annotated_is_archived?
    end
    ...
    can [:create, :update, :destroy], DynamicAnnotation::Field do |obj|
      obj.annotation.annotator_id == @user.id and !obj.annotated_is_archived?
    end
    can :update, [Dynamic, Annotation] do |obj|
      obj.get_team.include? @context_team.id and !obj.annotated_is_archived?
    end
    can :update, DynamicAnnotation::Field do |obj|
      obj.annotation.get_team.include? @context_team.id and !obj.annotated_is_archived?
    end
