...
class Request < ApplicationRecord
  ...
  def call_webhook(pm, title, summary, url)
    ...
    CheckSentry.notify(FeedRequestError.new("#{response.code} error calling webhook #{self.webhook_url}"), request_id: self.id, project_media_id: pm.id, url: url, response_body: response.body) if response.code.to_i >= 400
