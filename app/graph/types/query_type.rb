class QueryType < Types::BaseObject
  ...
  field :about, AboutType, description: 'Information about the application', null: true
  ...
  field :me, UserType, description: 'Information about the current user', null: true
  ...
  field :team, TeamType, description: 'Information about the context team or the team from given id', null: true do
  ...
  def team(**args)
    ...
    GraphqlCrudOperations.load_if_can(Team, tid, context)
  end
  ...
  field :project_media, ProjectMediaType, description: 'Information about a project media, The argument should be given like this: "project_media_id,project_id,team_id"', null: true do
  ...
  field :project_medias, ProjectMediaType.connection_type, null: true, connection: true do
  ...
  field :project, ProjectType, description: 'Information about a project, given its id and its team id', null: true do
  ...
  field :search, CheckSearchType, description: 'Search medias, The argument should be given like this: "{\"keyword\":\"search keyword\"}"', null: true do
    argument :query, String, required: true
  end

  def search(**args)
    Team.find_if_can(Team.current&.id.to_i, context[:ability])
  ...
  [:source, :user, :task, :tag_text, :bot_user, :project_group, :saved_search].each do |type|
    field type, description: "Information about the #{type} with given id" do
      type "#{type.to_s.camelize}Type".constantize
      argument :id, ID, required: true
    end

    def id(**args)
      GraphqlCrudOperations.load_if_can(type.to_s.camelize.constantize, args[:id], context)
    end
  end
end
