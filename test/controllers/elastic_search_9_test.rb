...
class ElasticSearch9Test < ActionController::TestCase
  ...
  test "should filter keyword by extracted text OCR" do
    ...
    stub_configs({ 'alegre_host' => 'http://alegre', 'alegre_token' => 'test' }) do
      ...
      WebMock.stub_request(:post, 'http://alegre/similarity/async/image').with(body: {doc_id: Bot::Alegre.item_doc_id(pm), context: {:has_custom_id=>true, :project_media_id=>pm.id, :team_id=>pm.team_id}, threshold: 0.89, url: "some/path", confirmed: false}).to_return(body: {
        "result": []
      }.to_json)
      WebMock.stub_request(:post, 'http://alegre/similarity/async/image').with(body: {doc_id: Bot::Alegre.item_doc_id(pm), context: {:has_custom_id=>true, :project_media_id=>pm.id, :team_id=>pm.team_id}, threshold: 0.95, url: "some/path", confirmed: true}).to_return(body: {
        "result": []
      }.to_json)
