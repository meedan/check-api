...
class GraphqlControllerTest < ActionController::TestCase
  ...
  test "should read project medias" do
    ...
    tg.assign_user(u.id)
    query = "query GetById { project_media(ids: \"#{pm.id},#{p.id}\") { tasks { edges { node { dbid } } }, tasks_count, created_at, language, language_code, last_status_obj {dbid}, annotations(annotation_type: \"comment,tag\") { edges { node { ... on Comment { dbid, assignments { edges { node { name } } }, annotator { ... on User { name } } } ... on Tag { dbid, assignments { edges { node { name } } }, annotator { ... on User { name } } } } } } } }"
  ...
  test "should read project medias with team_id as argument" do
    ...
    p = create_project team: @team
    pm = create_project_media project: p
    query = "query GetById { project_media(ids: \"#{pm.id},#{p.id},#{@team.id}\") { created_at, language, last_status_obj {dbid}, log_count } }"
    post :create, query: query
    assert_response :success
    assert_not_empty JSON.parse(@response.body)['data']['project_media']['created_at']
