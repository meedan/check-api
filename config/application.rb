...
module Check
  class Application < Rails::Application
    ...
    if cfg['locale'].blank?
      config.i18n.available_locales = ["ar","fr","pt","es","en"] # Do not change manually! Use `rake transifex:languages` instead, or set the `locale` key in your `config/config.yml`
    else
    ...
    config.middleware.insert_before 0, Rack::Cors do
      allow do
        ...
        resource '*',
          headers: [cfg['authorization_header'], 'Content-Type', 'Accept', 'X-Requested-With', 'Origin', 'Access-Control-Request-Method', 'Access-Control-Request-Headers', 'Credentials', 'X-Check-Client'],
