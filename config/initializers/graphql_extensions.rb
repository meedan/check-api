  ...
  module Relay
    class Edge
      def self.between(child_node, parent_node)
        relation = nil
        relation = Version.from_partition(parent_node.team_id).where(id: child_node.id) if child_node.is_a?(Version) && [ProjectMedia, ProjectSource].include?(parent_node.class)
