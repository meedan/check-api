source 'https://rubygems.org'

group :development, :test do
  gem 'byebug', require: false
  gem 'rubocop', '0.49.0', require: false
  gem 'rails-graphql-generator', require: false
end

group :test do
  gem 'parallel_tests'
  gem 'minitest-hooks'
  gem 'simplecov', '0.17.1', require: false
  gem 'simplecov-console', require: false
  gem 'codeclimate-test-reporter', '1.0.7', require: nil
  gem 'test_after_commit'
  gem 'minitest-retry'
end

group :doc do
  gem 'railroady', require: false
  gem 'sdoc', '2.0.3', require: false
  gem 'gem-licenses', require: false
end

group :development do
  gem 'awesome_print', require: false
  gem 'thin', require: false
  gem 'htmlentities', require: false
end

gem 'webmock'
gem 'mocha', '1.2.1'
gem 'pender_client', git: 'https://github.com/meedan/pender-client.git', ref: '89c9072'
gem 'sqlite3', '1.3.13', require: false
gem 'lograge'
gem 'rails', '5.0.2'
gem 'pg', '~> 0.18.0'
gem 'pg_application_name'
gem 'sass-rails', '~> 5.0'
gem 'uglifier', '>= 1.3.0'
gem 'coffee-rails', '~> 4.1.0'
gem "json", ">= 2.3.0"
gem 'jquery-rails'
gem 'turbolinks'
gem 'jbuilder', '~> 2.0'
gem 'cancancan', '~> 1.10'
gem 'bootstrap-sass', '~> 3.4.1'
gem 'carrierwave', '~> 1.3'
gem 'phony_rails'
gem 'airbrake', '11.0.1'
gem 'responders'
gem 'graphql', '1.12.12' #, '1.8.0.pre10'
gem 'devise', '4.8' #, '4.7.1'
gem 'devise-i18n', '1.10.0' #, '1.1.2'
gem 'devise-two-factor', '4.0.0' #, '3.0.3'
gem 'devise_invitable', '2.0.5'
gem 'omniauth-twitter'
gem 'omniauth-facebook'
gem 'omniauth-slack'
gem 'elasticsearch', '~> 7.x'
gem 'elasticsearch-api', '~> 7.x'
gem 'elasticsearch-transport', '~> 7.x'
gem 'elasticsearch-model', '7.1.1'
gem 'elasticsearch-persistence', '7.1.1'
gem 'paper_trail', '10.3.1'
gem 'graphiql-rails', git: 'https://github.com/meedan/graphiql-rails.git', ref: '8db0eac'
gem 'graphql-formatter'
#gem 'nokogiri', '1.11.4'
gem 'nokogiri', '1.11.7' #, '1.10.8'
gem 'puma'
gem 'rack-cors', '1.0.6', require: 'rack/cors'
gem 'sidekiq', '4.2.10'
gem 'sinatra', '2.1.0' #, '2.1.0'
gem 'pusher'
gem 'redis-namespace'
gem 'file_validators'
gem 'clamav-client'
gem 'filesize'
gem 'rest-client'
gem 'transifex-interface-ruby', git: 'https://github.com/meedan/transifex-interface-ruby.git'
gem 'http_accept_language', git: 'https://github.com/iain/http_accept_language.git'
gem 'i18n-generators'
gem 'rails-i18n' #, git: 'https://github.com/svenfuchs/rails-i18n.git', branch: 'rails-4-x', ref: 'f8c83ec'
gem 'ruby-cldr'
gem 'twitter'
gem 'twitter-text'
gem 'request_store'
gem 'geojsonlint'
gem 'jsonpath', '~> 0.8.8'
gem 'graphql-batch', '0.3.5'
gem 'google-protobuf', '3.17.2'
gem 'concurrent-ruby', require: 'concurrent'
gem 'apollo-tracing', '1.5.0'
gem 'rubyzip', '1.3.0'
gem 'rack-protection', '2.1.0' #, '2.0.1'
gem 'loofah', '2.10.0' #, '~> 2.10'
gem 'deep_cloneable', '~> 3.1'
gem 'rails-html-sanitizer', '~> 1.2'
gem 'sprockets', '3.7.2'
gem 'pghero', '2.8.1' #, '2.8.1'
gem 'pg_query', '0.13.0'
gem 'multi_json', '1.15.0' #, '1.12.2'
gem 'json-schema', '2.5.0'
gem 'ffi', '1.9.24'
gem 'google_drive'
gem 'activerecord-import', '1.1.0' #, '1.1.0'
gem 'redis-rails'
gem 'activejob', '5.0.2'
gem 'actionmailer', '5.0.2'
gem 'actionpack', '5.0.2'
gem 'activerecord', '5.0.2'
gem 'rack', '2.2.3'
gem 'jwt'
gem 'smooch-api', '5.29.1'#, git: 'https://github.com/smooch/smooch-ruby.git', ref: 'a2c1646'
gem 'typhoeus'
gem 'sidekiq-benchmark'
gem 'sidekiq-status'
gem 'mini_magick', '4.10.1'
gem 'peek', '1.1.0' #, '1.0.1'
gem 'peek-rblineprof'
gem 'request_store-sidekiq', '~> 0.1'
gem 'aasm'
gem 'redis-objects'
gem 'connection_pool'
gem 'useragent', '~> 0.16.10'
gem 'authtrail'
gem 'omniauth-google-oauth2', git: 'https://github.com/meedan/omniauth-google-oauth2.git', ref: 'e39e2d2'
gem 'rqrcode'
gem 'fog-aws'
gem 'aws-sdk-s3', '1.46.0'
gem 'therubyracer'
gem 'execjs'
gem 'carrierwave-video-thumbnailer'
gem 'carrierwave_backgrounder', git: 'https://github.com/meedan/carrierwave_backgrounder'
gem 'activerecord-redshift-adapter' #,  git: 'https://github.com/arp/activerecord-redshift-adapter.git', branch: "rails4-compatibility"
# gem 'partitioned', git: 'https://github.com/meedan/pconcurrent-rubyartitioned.git', branch: 'rails-4-2'
gem 'honeycomb-beeline'
gem 'bumbler', require: false
gem 'swagger-docs', '0.2.9'
gem 'ruby-progressbar', '~> 1.10', '>= 1.10.1'
gem 'shortener'
gem 'taglib-ruby'
gem 'bitly', require: false
gem 'open_uri_redirections', require: false
gem 'jsonapi-resources'
gem 'where-or'
