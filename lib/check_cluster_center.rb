class CheckClusterCenter
  ...
  def self.replace_or_keep_cluster_center(current_center, new_pm)
    ...
    # Check #1: Fact-check
    passed = self.fact_check_condition(current_center, new_pm)
    ...
    passed = self.requests_count_condition(current_center, new_pm)
    return passed.first if passed.length == 1
    # Check #4: Most recently Last updated
    passed = self.most_recent_condition(current_center, new_pm)
    return passed.first if passed.length == 1
  ...
  def self.fact_check_condition(current_center, new_pm)
    passed = []
  ...
  def self.requests_count_condition(current_center, new_pm)
    passed = []
  ...
  def self.most_recent_condition(current_center, new_pm)
    passed = []
