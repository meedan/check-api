namespace :check do
  namespace :migrate do
    task update_secondary_report_status: :environment do
      old_logger = ActiveRecord::Base.logger
      ActiveRecord::Base.logger = nil
      started = Time.now.to_i
      errors = 0
      conditions = ['r.relationship_type = ? OR r.relationship_type = ?', Relationship.confirmed_type.to_yaml, Relationship.suggested_type.to_yaml]
      ...
      ProjectMedia.where(conditions).joins(joins).find_in_batches(batch_size: 3000) do |pms|
        ...
        pms.each do |pm|
          begin
            ...
            Rails.cache.delete "check_cached_field:ProjectMedia:#{pm.id}:report_status"
            puts "Report status: #{pm.report_status}"
          rescue
            errors += 1
          end
        end
