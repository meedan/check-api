namespace :check do
  namespace :migrate do
    task fix_titles: :environment do
      ...
      i = 0
      total = 0
      n = PaperTrail::Version.joins("INNER JOIN annotations a ON a.id::text = versions.item_id AND versions.item_type = 'Annotation' INNER JOIN project_medias pm ON pm.id = a.annotated_id AND a.annotated_type = 'ProjectMedia' AND a.annotation_type = 'metadata'").where('pm.id <= ?', last_id).group('pm.id, versions.id').count.size
