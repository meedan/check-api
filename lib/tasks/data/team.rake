namespace :check do
  namespace :team do
    ...
    task :delete_teams_cache_and_es_values_by_slug, [:slugs] => :environment do |_t, args|
      slugs = args[:slugs].to_s.split('|')
      last_team_id = Rails.cache.read('check:team:delete_teams_cache_and_es_values_by_slug') || 0
      Team.where('id > ?', last_team_id).where(slug: slugs).find_each do |team|
        puts "\nProcessing team: #{team.slug}\n"
        clean_cache_and_es(team)
        Rails.cache.write('check:team:delete_teams_cache_and_es_values_by_slug', team.id)
      end
    ...
    task :delete_teams_cache_and_es_values, [:date, :exclude] => :environment do |_t, args|
      ...
      Team.where('id > ?', last_team_id).where.not(slug: slugs).find_each do |team|
        ...
        Rails.cache.write('check:team:delete_teams_cache_and_es_values', team.id)
      end
      Rails.cache.delete('check:team:delete_teams_cache_and_es_values')
      puts "\n Teams list \n"
      pp output
    end
  end
