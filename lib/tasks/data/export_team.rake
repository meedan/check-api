  ...
  namespace :data do
    ...
    task :export_team, [:team] => :environment do |task, args|
      ...
      team = if args.team.to_i > 0
        Team.find_by(id: args.team)
      else
        Team.find_by(slug: args.team)
      end
      team_id = team.id
      filename = "check-#{team.slug}.sqlite3"
      begin
        File::unlink(filename)
        puts "Deleted existing #{filename}"
      rescue Errno::ENOENT
        # Common case
      end
      begin
        File::unlink("#{filename}.lz4")
        puts "Deleted existing #{filename}.lz4"
      rescue Errno::ENOENT
        # Common case
      end
      SQLite3::Database.new(':memory:') do |db|
        ...
        db.execute("VACUUM INTO '#{filename}'")
      end
      ...
      puts "Compressing #{filename}.lz4..."
      `lz4 --favor-decSpeed #{filename} #{filename}.lz4`

      puts "Done! We left #{filename} around for easy debugging.\nUpload #{filename}.lz4 to Workbench."
    end
  end
end
