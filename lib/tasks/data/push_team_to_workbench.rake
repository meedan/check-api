...
def raise_on_http_problem(response)
...
def get_s3_config_from_workbench(workflow_id, step_id, api_token)
...
def upload_file_to_s3(path, s3_config)
...
def finish_workbench_upload(finish_url, api_token, filename)
...
def upload_file_to_workbench(workflow_id, step_id, api_token, path, filename)
  ...
  namespace :data do
    ...
    task :push_team_to_workbench, [:team_slug, :workflow_id, :step_id, :api_token] => :environment do |task, args|
      ...
      Tempfile.open("#{args.team_slug}-sqlite3-lz4") do |f|
        ...
        upload_file_to_workbench(
          args.workflow_id,
          args.step_id,
          args.api_token,
          f.path,
          "#{args.team_slug}-#{Date.today.iso8601}.sqlite3.lz4"
        )
      end
    end
  end
